...
// --max-lazy велит игнорировать подсказки о немедленной компиляции: если скобки
// работают именно как подсказка, под этим флагом разница должна исчезнуть.
const maxLazyEvents = functionEvents(subject, ['--max-lazy']);
const firstEventIn = (events_, name) => events_.find((entry) => entry.name === name)?.label ?? '—';

table(
  ['функция', 'обёртка в исходнике', 'первое событие', 'то же с --max-lazy'],
  wrappers.map(([name, wrapper]) => [
    name,
    wrapper,
    firstEventIn(events, name),
    firstEventIn(maxLazyEvents, name),
  ]),
);
...
note(
  ...
  '',
  'Последняя колонка подтверждает, что дело именно в подсказке, а не в чём-то другом:',
  'под флагом `--max-lazy` («игнорировать подсказки о немедленной компиляции») разница',
  'между обёртками исчезает, и все функции начинают с предразбора.',
  '',
